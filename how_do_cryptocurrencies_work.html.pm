...
◊(define-meta updated "2021-03-10T19:05:19+01:00")
...
As stated in the introduction, the focus isn't on technical details, but it's a tight balance to make between keeping it simple and explaining how cryptocurrencies work. If this chapter is too technical, you can safely skip to the ◊link[next-chapter]{next chapter} or just read ◊link[summary]{the summary}---it's not required knowledge.
...
◊ndef["decentralized"]{
    Decentralization is a common term used to refer to the lack of a trusted third-party. Instead, multiple unrelated entities come together and decide as a group.
...
This is called a ◊def{Sybil attack}. Think of how one person can use multiple online identities to troll or attack people online; it's hard to know who is real.◊mn{trolls}
...
If you've heard about cryptocurrencies, then maybe you've also heard about ◊def{cryptocurrency miners} or ◊def{Bitcoin miners}. This is how Bitcoin provides sybil resistance and prevents double spends.
...
◊ul{
    ◊li{◊strong{Hard enough}
        ...
        An example is playing a game of Yahtzee, where the goal is to cast dice and get them all to have the same number, and you change the number of dice in play.
    }
    ...
    ◊li{◊strong{Memoryless}
        ...
        Knowing earlier outcomes shouldn't give any advantage. Finding a solution should essentially be random.
...
The transactions must follow common rules, called ◊def{consensus rules}, otherwise other miners and users who use the blockchain will discard the block. For example, a transaction cannot send coins from an empty address or spend coins without having access to the private key of that address.

In return for adding the block, you get to collect the rewards. One for finding a block, and you can collect transaction fees for the transactions you include in the block.◊mn{blockreward}
...
◊ndef["node-types"]{
    There are several different ways to interact with the blockchain. For the end user there are three main ways, with different trade-offs:
...
◊ndef["blockreward"]{
    As I'm writing this in February 2021, the current ◊def{block reward} for Bitcoin is 6.25 BTC, or around $250,000. With one block expected every 10 min that's about $36,000,000 per day. Bitcoin mining is big business.
}
...
◊ndef["0-conf"]{
    You can even accept transactions without any confirmation, called ◊def{0-conf}. They are much less secure than a confirmed transaction, but since most miners respect the first-seen-rule it's fairly safe for small purchases.
...
◊ndef["more-energy"]{
    There is some correlation here. Because each transaction contains a fee the miner can claim, more transactions means the reward is bigger which, in turn, supports more miners. But the reverse is not true, more energy does not mean higher transaction throughput.
}
...
Some cryptocurrencies, like Monero and Bitcoin Cash, have regular network upgrades where consensus rules are changed.◊mn{hard-soft}
...
◊ndef["hard-soft"]{
    ...
    Instead, a soft-fork doesn't break older node implementations. They will simply ignore the new soft-fork rules---they will not fully validate the chain anymore but they will follow it. The rules are instead enforced by the miners who must upgrade. For example, SegWit in Bitcoin was implemented using a soft-fork.
}

Because a network upgrade is a fork, there will be two chains (as long as someone mines them). Sometimes, the minority chain lives on as a new cryptocurrency; Ethereum Classic is the continuation of the old chain ◊link[dao]{after an Ethereum fork}.
...
◊link[bitcoin_whitepaper]{Bitcoin's white paper} is always a good place to begin, and there are also many good resources online. I've tried to include key concepts which you can use as starting points in your search.
