...
Money is perhaps the most obvious usage of decentralized byzantine fault tolerance, which is the core making cryptocurrencies work, but there are other usages. In this section I'll highlight some of the them and we'll see that we can build other functionality on top of existing cryptocurrencies.

◊subhead{Embedding data}

The first thing we can observe is that it's possible to insert data into cryptocurrencies, essentially making the data immutable. It's not especially important exactly how, but if you're curious here are some ways:
...
This is for example the script of a standard Bitcoin transaction:

◊code{
    OP_DUP OP_HASH160 <pubKeyHash> OP_EQUALVERIFY OP_CHECKSIG
}

The scripting language in Bitcoin is fairly limited but the scripting language in Ethereum is much more powerful and can do more things. You can for example create games where you ◊link[cryptokitties]{buy and sell cats} on Ethereum.◊sn{tradeoffs}
